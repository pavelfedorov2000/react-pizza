...
function Cart() {
    ...
    const onClearCart = () => {
        if (window.confirm('Вы действительно хотите очистеть корзину?')) {
            dispatch(clearCart());
        }
    }
        ...
        <div className="container container--cart">
            {totalCount != 0 ? <div className="cart">
                ...
                <div className="content__items">
                    {
                        addedPizzas.map(obj => <CartItem key={obj.id} id={obj.id} imageUrl={obj.imageUrl} name={obj.name} type={obj.type} size={obj.size} totalCount={items[obj.id].items.length} totalPrice={items[obj.id].totalPrice} onRemove={onRemoveItem} onMinus={onMinusPizza} onPlus={onPlusPizza} />)
                    }
                </div>
